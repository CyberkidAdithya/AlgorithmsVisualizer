...
class DrawInformation:
	...
	def __init__(self, width, height, lst):
		...
		pygame.display.set_caption("Sorting Alorithm Visualizer")
		self.set_list(lst)
...
def draw(draw_info, algo_name, ascending, sorting_speed):
	draw_info.window.fill(draw_info.BACKGROUND_COLOR)

	title = draw_info.LARGE_FONT.render(f"{algo_name} - {'Ascending' if ascending else 'Descending'} - Speed:{sorting_speed}", 1, draw_info.BLUE)
...
def selection_sort(draw_info, ascending = True):
	...
	for i in range(len(lst)):
		...
		for j in range(i + 1, len(lst)):
			draw_list(draw_info, {j: draw_info.GREEN, idx_min: draw_info.RED}, True)

			if (lst[idx_min] > lst[j] and ascending) or (lst[idx_min] < lst[j] and not ascending):
				idx_min = j
				yield True

		lst[i], lst[idx_min] = lst[idx_min], lst[i]
		draw_list(draw_info, {i: draw_info.GREEN, idx_min: draw_info.RED}, True)
	return lst
...
def main():
	...
	while run:
		...
		for event in pygame.event.get():
			...
			elif event.key == pygame.K_LSHIFT and not sorting:
				speed = max(60, (speed + 20) % 140)
